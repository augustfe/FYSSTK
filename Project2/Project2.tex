...
For some problems, such as classification, a linear regression model such as OLS does not work properly. In classification problems, the variables are discrete (i.e. categories) and the goal is not to predict a response to continuous variables. Assume that we have two different categories, $\{1,0\}$ and let
\begin{equation*}
    \mathbf{\hat{y}} = \hat{f}(\mathbf{x}_i) = \mathbf{X}\bm{\beta}
\end{equation*}
as in the previous sections. By using the logistic function, we can compute the probability of whether any data point is in the first or second category.
\begin{equation*}
    p(\mathbf{X}) = \frac{e^{\mathbf{X}\bm{\beta}}}{1 + e^{\mathbf{X}\bm{\beta}}} 
\end{equation*}
This function is a transformation of the linear regression model to guarantee that the output is a probability. 
Temporary notes
\begin{itemize}
...
The two natural weaknesses of gradient descent are neatly explained in \cite[p.~65--71]{MLRefined}. The first stems from the gradient always being perpendicular to the contour line. This is easily proved through parameterization.
...
\subsubsection{Minimizing the cost function: Back propagation}

Once obtaining the values of our output layer $\mathcal{N}(\boldsymbol{x})$ its immediately natural to evaluate how well our model performed. That is the loss from the expected output values
...
Finding these partial derivatives is the key to our algorithm.

 We return to $a_i^l$ begin the value of node $i$ in layer $l$, $z_i^l$ the value before activation. Then for some arbitrary layer $l$ we get by the chain rule

\begin{align*}
    \frac{\partial C}{\partial w_{i,j}^l} =& \frac{\partial C}{\partial a_i^l} \frac{\partial f^l}{\partial a_i^l} \frac{\partial z_i^l}{\partial w_{i,j}^l} \\
    \frac{\partial C}{\partial b_i^l} = & \frac{\partial C}{\partial a_i^l} \frac{\partial f^l}{\partial a_i^l} \frac{\partial z_i^l}{\partial b_i^l}
\end{align*}
The third factors of these products simplifies to

\begin{align*}
    \frac{\partial z_{i}^l }{\partial w_{i,j}^l} = &  \frac{\partial}{\partial w_{i,j}^l} \sum_{k = 0}^{N_l - 1} w_{i,k}^L a_k^{l-1} + b_i^l = a_j^{l-1}\\
    \frac{\partial z_i^l}{\partial b_i^l} = 1
\end{align*}
\noindent
The first $\frac{\partial C}{\partial a_i^l}$ can be expanded using the chain rule again.

\begin{align*}
    \frac{\partial C}{\partial a_i^l} = \sum_{j=0}^{N_{l+1}-1} \frac{\partial C}{\partial a_j^{l+1}} \frac{\partial_i}{}
\end{align*}
...
Gathering $\frac{\partial C}{\partial a_i^l}$ $\frac{\partial f^l}{\partial a_i^l}$ to one variable $\delta_{i,j}^{l}$ we get
\begin{align*}
    \frac{\partial C}{\partial w_{i,j}^l} =& \delta_{i,j}^{l} a_j^{l-1} \\
    \frac{\partial C}{\partial b_i^l} = & \delta_{i,j}^{l} 
\end{align*}
