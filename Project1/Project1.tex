...
Ridge regression is a method which aims to address these issues from OLS, by taking into account the size of the values in the regression parameter $\boldsymbol{\beta}$. The cost function is now defined as
\begin{align*}
...
Given a $\lambda$ our optimal regression parameter $\boldsymbol{\hat{\beta}}$ satisfies
\begin{equation*}
...
\subsection{Lasso}
LASSO is an abbreviating of Least Absolute Shrinkage and Selection Operator, and performs both regularization as in Ridge regression, and variable selection. Here, the cost function is defined as
\begin{align*}
    C \left( \boldsymbol{X}, \boldsymbol{\beta} \right) &= \frac{1}{n} \lVert \boldsymbol{y} - \boldsymbol{X\beta} \rVert_2^2 + \lVert \boldsymbol{\beta} \rVert_1.
\end{align*}
