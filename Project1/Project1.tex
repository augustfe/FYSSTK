...
Throughout the project, we use packages such as \mintinline{python}{numpy} and \mintinline{python}{scikit-learn} in order to speed up and simplify our code. In order to generate our data points, we use the following method
\begin{minted}{python}
import numpy as np
...
\end{minted}
This initialises our data points as \mintinline{python}{numpy}-arrays, allowing for vectorized code.

% For each of our functions, we create 

% Implemented in Python, $\boldsymbol{x}$, $\boldsymbol{y}$ and $\boldsymbol{\varepsilon}$ are all numpy arrays with length $n$. $\ell$
