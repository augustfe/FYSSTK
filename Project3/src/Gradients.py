...
class Gradients:
    ...
    def __init__(
        ...
        if not isinstance(n, int):
            raise TypeError(f"n should be an integer, not {
                            n} of type {type(n)}")
        if not (n == len(x) == len(y)):
            raise ValueError(
                "Number of points must correspond to length of arrays")
    ...
    def StochasticGradientDescent(
        ...
        for epoch in range(n_epochs):
            ...
            self.errors = assign(
                self.errors, epoch, self.cost_func(
                    self.X, self.y, theta, self.lmbda)
            )
