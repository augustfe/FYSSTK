...
@jit
def setup_ones(a_l: np.ndarray) -> np.ndarray:
    return np.hstack([np.ones((a_l.shape[0], 1)), a_l])
...
class NeuralNet2:
    ...
    def __init__(
        ...
        if not isinstance(seed, int) or seed is None:
            raise TypeError(
                f"Seed must be either None or int, not {type(seed)}")
        if any(dimension <= 0 for dimension in dimensions):
            raise ValueError(
                f"Number of dimensions must be positive, not {dimensions}")
    ...
    def reset_weights(self) -> None:
        ...
        for i in range(len(self.dimensions) - 1):
            ...
            # Bias
            weight_array[0, :] = onp.random.randn(
                self.dimensions[i + 1]) * 0.01
    ...
    def fit(
        ...
        if not isinstance(batches, int):
            raise TypeError(
                f"Number of batches must be int, not {type(batches)}")
        if not isinstance(epochs, int):
            raise TypeError(
                f"Number of epochs must be int, not {type(epochs)}")

        # Handle ValueErrors
        if batches <= 0:
            raise ValueError(
                f"Number of batches must be positive, not {batches}")
        if epochs <= 0:
            raise ValueError(
                f"Number of epochs must be positive, not {epochs}")
        if lmbda < 0:
        ...
        for e in range(epochs):
            """
            batches could be lower than it should be
            example: X_train.shape[0] = 100. We have 100 inputs. And
            a batches is 51.Then 
            batchsize =  100//51 = 1. Meaning there is only a single point in our 
            batch. We should then perform 100 itterations in the inner loop not 
            51. Because at that size 100 bathces would fit. Which is how its presented
            on wiki and in the foils

            """
            for _ in range(batches):  # batches could be lower than what it should be
    ...
    def feed_forward(self, X_batch: np.ndarray) -> np.ndarray:
        ...
        # Make sure X is a matrix, the case where we only have a single sample
        if len(X_batch.shape) == 1:
            X_batch = X_batch.reshape((1, X_batch.size))

        # Add a bias
        # X_batch = setup_ones(X_batch)
